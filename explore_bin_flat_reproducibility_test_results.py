...
for i, test_res in enumerate(all_test_results):
	...
	delta_results = pd.DataFrame(data=delta_results,columns = ['MAE_returned_vector_distances', 'n_vectors_returned_in_both_search', 'distance_delta_for_vecs_returned_in_both_search'])
	delta_between_different_indices.append(delta_results.n_vectors_returned_in_both_search.value_counts())

	distance_delta_for_vecs_returned_in_both_search = pd.Series(np.hstack(delta_results.distance_delta_for_vecs_returned_in_both_search.apply(np.asarray).values)).describe()
	distance_delta_for_vecs_returned_in_both_search.name = 'distance_delta_for_vecs_returned_in_both_search'
	delta_different_indices_same_embeddings_distance_difference.append(distance_delta_for_vecs_returned_in_both_search)
...
delta_different_indices_same_embeddings_distance_difference_df = pd.DataFrame(delta_different_indices_same_embeddings_distance_difference)
delta_different_indices_same_embeddings_distance_difference_df.index = test_uuids
delta_different_indices_same_embeddings_distance_difference_df.columns = pd.MultiIndex.from_tuples([(delta_different_indices_same_embeddings_distance_difference[0].name,c) for c in delta_different_indices_same_embeddings_distance_difference_df])
...
df_summary_results = df_summary_results.join(delta_different_indices_same_embeddings_distance_difference_df)
...
cols_highlight = [('','n_vec_index'),
				...
				('distance_delta_for_vecs_returned_in_both_search', 'mean'),
            ('distance_delta_for_vecs_returned_in_both_search',  'std'),
            ('distance_delta_for_vecs_returned_in_both_search',  'min'),
            ('distance_delta_for_vecs_returned_in_both_search',  'max')
				]
